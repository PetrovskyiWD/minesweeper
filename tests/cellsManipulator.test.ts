import { CellState, Field } from '../src/helpers/field'
import { incrementNeighbours, getNeighboursItems, checkItemInField } from '../src/helpers/cellsManipulator'

const { empty, bomb } = CellState

describe('Check item in field', () => {
  ...
  describe('Big field', () => {
    ...
    it('In x and y range', () => {
      expect(checkItemInField([2, 3] , field)).toBe(true)
    })
  })
})
...
describe('Check Increment Neighbours', () => {
  describe('Simple cases', () => {
    ...
    it('Fields 2x2 with one mine', () => {
      expect(incrementNeighbours([0, 0], [[bomb, empty], [empty, empty]])).toStrictEqual([[bomb, 1], [1, 1]])
    })

    it('Fields 2x2 with two mines', () => {
      expect(incrementNeighbours([0, 0], [[bomb, empty], [empty, bomb]])).toStrictEqual([[bomb, 1], [1, bomb]])
    })

    it('Fields 3x3 with one mine', () => {
      expect(incrementNeighbours(
        [1, 1],
        [
          [empty, empty, empty],
          [empty, bomb, empty],
